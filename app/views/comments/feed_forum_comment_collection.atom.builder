...
atom_feed do |feed|
	feed.title('Birdstack: ' + @comment_collection.title)
	...
	for comment in @comment_collection.comments.reverse
		feed.entry(comment, {:url => url_for(:controller => 'forums', :forum => comment.comment_collection.forum.url, :action => 'thread', :id => comment.comment_collection.id)}) do |entry|
