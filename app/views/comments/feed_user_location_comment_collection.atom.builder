...
atom_feed do |feed|
	feed.title("Birdstack: #{@comment_collection.user_location.name} (#{@comment_collection.user.login})")
	...
	for comment in @comment_collection.comments.reverse
		feed.entry(comment, {:url => location_path(:user => @comment_collection.user.login, :id => @comment_collection.user_location.id)}) do |entry|
